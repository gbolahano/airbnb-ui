import data from "../mock/data/listingData";
import { filters } from "../mock/data/listingData";
import {
  HeartIcon,
  FunnelIcon,
  GlobeAltIcon,
...
const Home = () => {
  ...
  const addToFavorite = (id: number) => {
    ...
    if (favorites.includes(id)) {
      ...
      if (index > -1) {
        let newFavorites = copyFavorites.filter(
  ...
  return (
    <div className="pb-12">
      <div
        className={`sticky w-full top-0 z-10 bg-white border-b shadow-md ${
          scrollPosition > 0 ? "shadow-md" : "sm:border-0 sm:shadow-none"
        }`}
      >
        <div className="border-b border-b-slate-200">
          <div className="py-4 mx-auto max-w-7xl px-6 flex items-center justify-between">
            <div>
              <svg
                width="102"
                height="32"
            ...
            <div className="hidden lg:block">
              <button className="flex items-center rounded-full shadow hover:shadow-md transition duration-300 ease-in-out space-x-2 border px-4 py-1.5">
                <div className="font-semibold border-r pl-2 pr-4 whitespace-nowrap">
                  Anywhere
                </div>
                <div className="font-semibold border-r pl-2 pr-4 whitespace-nowrap">
                  Any week
                </div>
                <div className="pl-3 text-gray-500 whitespace-nowrap">
                  Add guests
                </div>
                <MagnifyingGlassCircleIcon className="text-[#FF5A5F] h-10 w-10" />
              </button>
            </div>
            <div className="flex items-center">
              <button className="text-gray-700 px-3 py-2.5 rounded-full hover:bg-gray-100 font-semibold transition duration-300 ease-in-out">
                Become a Host
              </button>
              <button className="rounded-full hover:bg-gray-100 font-semibold transition duration-300 ease-in-out px-3 py-2.5 mr-1">
                <GlobeAltIcon className="h-5 w-5 text-gray-700" />
              </button>
              <button className="flex items-center rounded-full shadow-sm hover:shadow-md transition duration-300 ease-in-out space-x-2 border px-3 py-1.5">
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex space-x-4 pt-6 bg-white">
            <div className="flex space-x-9 overflow-x-auto scrollbar-none">
              {filters.map((filter) => {
                const { Icon } = filter;
                return (
                  <button
                    ...
                    className={`flex flex-col items-center cursor-pointer space-y-2 pb-3 hover:border-b-2 group transition duration-300 ease-in-out ${
                      active === filter.id
                        ? "border-b-2 border-gray-900 hover:border-gray-900"
                        : "hover:border-gray-300"
                    }`}
                  >
                    <Icon
                      className={`text-gray-600 h-5 w-5 group-hover:text-gray-900 ${
                        active === filter.id ? "text-gray-900" : ""
                      }`}
                    />
                    <div
                      className={`text-sm text-gray-500 group-hover:text-gray-900 whitespace-nowrap ${
                        active === filter.id ? "text-black" : ""
                      }`}
                    >
                      {filter.name}
                    </div>
            ...
            <button
              type="button"
              className="text-gray-700 hover:bg-white-800 focus:outline-none focus:ring-2 focus:ring-slate-800 font-medium border border-gray-200 rounded-lg text-sm px-5 text-center mr-2 mb-2 flex items-center space-x-1 self-center py-3"
            >
      ...
      <div className="mx-auto max-w-7xl px-6">
        <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6  z-0">
          {data.map((listing) => {
            return (
              <div key={listing.id} className="cursor-pointer">
                <div className="relative">
                  ...
                  {favorites.includes(listing.id) ? (
                    <HeartSolidIcon
                      ...
                      className="h-7 w-7 absolute top-4 right-4 text-[#FF5A5F] cursor-pointer"
                    />
                  ) : (
                    <HeartIcon
                      onClick={() => addToFavorite(listing.id)}
                      className="h-7 w-7 absolute top-4 right-4 text-white cursor-pointer"
                    />
                  )}
                </div>
                <div className="mt-2">
                  <div className="flex justify-between">
                    ...
                    <p className="flex items-center">
                      <StarIcon className="h-4 w-4 text-gray-700 mr-1" />
                      {listing.rating}
                    </p>
                  </div>
                  <p className="text-gray-600">{listing.distance}</p>
                  <p className="text-gray-600">{listing.date}</p>
                  <div className="flex mt-1">
                    <p className="font-semibold mr-1">{listing.price}</p>
                    night
                  </div>
                </div>
              </div>
